...
function describeHostWaitFailure(result, phase) {
  if (result.status === 'fatal-frontend-error') {
    ...
    return `Windows dev verify hit a frontend exception while waiting for ${phase}. ${detail}`;
  }

  return `Windows dev verify timed out waiting for ${phase}.`;
}
...
async function main() {
  ...
  try {
    ...
    if (ready.status !== 'matched') {
      ...
      throw new Error(`${describeHostWaitFailure(ready, 'Metro-backed host readiness')}\n${tail}`);
    }
    ...
    if (smokeReady.status !== 'matched') {
      ...
      throw new Error(`${describeHostWaitFailure(smokeReady, 'challenge-advisor dev smoke completion')}\n${tail}`);
    }
